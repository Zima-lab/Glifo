...
- **Tecnica** — 32 voci fra tecnologie di stampa (dal punzone al digitale), tipometria (punto Didot e Pica, tipometro), parametri di composizione e correzioni ottiche
...
- **Sette tavole illustrate** in stile enciclopedia ottocentesca, più **tredici figure** sulle correzioni ottiche e la percezione (una per voce)
...
## Le figure delle correzioni ottiche

Tredici figure nello stile delle tavole, una per concetto, in `plates/perc-*.svg`.
Le genera `tools/p9_percezione.py`. Si numerano a parte (« Fig. », non « Tav. »)
grazie al campo `fig` in `PLATES`. Dove una figura riporta un numero, è misurato
sul contorno vero dell'Archivo Bold; dove una correzione è esagerata per
renderla visibile, la didascalia lo dichiara.
